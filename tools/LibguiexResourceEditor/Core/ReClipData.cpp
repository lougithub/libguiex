...
// General.
// -------------------------------------------------------------------------
int ReClipData::ms_nextId = 0;


ReClipData::ReClipData()
: m_isFlushingData( false )
{
	m_name = QString( "CLip_%1" ).arg( ms_nextId++ );
}
...
//		
//		m_name	= model->data( model->index( idx.row(), 0, QModelIndex() ) ).toString();
//		qreal x	= model->data( model->index( idx.row(), 1, QModelIndex() ) ).toDouble();
//		qreal y	= model->data( model->index( idx.row(), 2, QModelIndex() ) ).toDouble();
//		qreal w	= model->data( model->index( idx.row(), 3, QModelIndex() ) ).toDouble();
//		qreal h	= model->data( model->index( idx.row(), 4, QModelIndex() ) ).toDouble();		
//
//		if( x >= 0.0f && x < 1.0f )
//			m_offset.setX( x );
//		if( y >= 0.0f && y < 1.0f )

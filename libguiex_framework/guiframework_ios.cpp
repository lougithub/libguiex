...
namespace guiex
{
	...
	int32 CGUIFramework_IOS::InitializeSystem( const CGUIIntSize& rScreenSize, const char* pDataPath )
	{
		...
		if( GSystem != NULL )
		{
			CGUIException::ThrowException( "[CGUIFramework_IOS::InitializeSystem] GSystem has been created!" );
			return -1;
		}
	...
	void CGUIFramework_IOS::ReleaseSystem()
	{
		...
		if( GSystem == NULL )
		{
			CGUIException::ThrowException( "[CGUIFramework_IOS::ReleaseSystem] GSystem has been released!" );
			return;
		}
		GSystem->Release();
		delete GSystem;

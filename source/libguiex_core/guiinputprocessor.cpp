...
namespace guiex
{
	...
	*/
	...
	public:
		struct 
		{
			CGUITimer		m_aTimer;			//!< Timer used to track clicks for this button.
...
namespace guiex
{
	...
	CGUIInputProcessor::CGUIInputProcessor()
		...
		,m_nDbClickTimeout(GUI_DBCLICK_TIME)
		,m_pSystem(NULL)
	{
	...
	bool	CGUIInputProcessor::OnMouseButtonDown(const IGUIInterfaceMouse::SMouseEvent& rMouseEvent)
	{
		...
		if( !m_pMouseTracker->m_aButtonTrack[rMouseEvent.m_eButton].m_pTargetWidget ||
			...
			m_pSystem->GetGlobalTimer() - m_pMouseTracker->m_aButtonTrack[rMouseEvent.m_eButton].m_aTimer > static_cast<int32>(m_nDbClickTimeout))
		{
			//reset mouse tracker
		...
		//re-initialize timer
		m_pMouseTracker->m_aButtonTrack[rMouseEvent.m_eButton].m_aTimer = m_pSystem->GetGlobalTimer();
		...
		if( pTargetWidget )
		{
			...
			case 2:
				if( pTargetWidget->GetFlag(CGUIWidget::eFLAG_EVENT_DBCLICK))
				{
			...
			default:
				if( pTargetWidget->GetFlag(CGUIWidget::eFLAG_EVENT_MULTICLICK))
				{
			...
			//for drag event
			if( pTargetWidget->GetFlag(CGUIWidget::eFLAG_MOVABLE) &&
	...
	bool	CGUIInputProcessor::OnMouseButtonUp(const IGUIInterfaceMouse::SMouseEvent& rMouseEvent)
	{
			...
			pTargetWidget == m_pMouseTracker->m_aButtonTrack[rMouseEvent.m_eButton].m_pTargetWidget )
		{
			if( m_pMouseTracker->m_aButtonTrack[rMouseEvent.m_eButton].m_uLastDownEvent == eEVENT_MOUSE_DOWN &&
				pTargetWidget->GetFlag(CGUIWidget::eFLAG_EVENT_CLICK))
			{
				//click event
				CGUIEventMouse aEventMouse;
